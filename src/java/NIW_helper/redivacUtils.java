...
public class redivacUtils {
    ...
    * add new data
    ...
    public Boolean addRedivac(String datas) {
        ...
        String pmi, hfc, episodeDate, encounterDate, radivac_drain_date, shift, total_in_bottle, amount_increase_shift, total_in_bottle_es, remarks, assignBy;
    ...
    * get data from DB
    ...
    public ArrayList<ArrayList<String>> getRedivac(String datas) {
        ...
        if (viewBy.equalsIgnoreCase("today")) {
            ...
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and DATE(radivac_drain_date) ='" + dateTime + "' order by radivac_drain_date;";

        } else if (viewBy.equalsIgnoreCase("yesterday")) {
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and  DATE(radivac_drain_date) = SUBDATE('" + dateTime + "',1) order by radivac_drain_date;";

        } else if (viewBy.equalsIgnoreCase("7day")) {
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and DATE(radivac_drain_date) between SUBDATE('" + dateTime + "',7) and '" + dateTime + "' order by radivac_drain_date ;";

        } else if (viewBy.equalsIgnoreCase("30day")) {
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and DATE(radivac_drain_date) between SUBDATE('" + dateTime + "',30) and '" + dateTime + "' order by radivac_drain_date;";

        } else if (viewBy.equalsIgnoreCase("60day")) {
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and DATE(radivac_drain_date) between SUBDATE('" + dateTime + "',60) and '" + dateTime + "' order by radivac_drain_date ;";

        } else if (viewBy.equalsIgnoreCase("custom")) {
            String dateSplit[] = dateTime.split("\\^", -1);
            startDate = dateSplit[0];
            endDate = dateSplit[1];
            sql = "SELECT pmi_no,hfc_cd,episode_date,encounter_date,DATE_FORMAT(DATE(radivac_drain_date),'%d/%m/%Y'),shift,total_in_bottle,amount_increase_shift,total_in_bottle_es,remarks,assign_by FROM lhr_ort_niw_redivac_drain_chart where pmi_no ='" + pmino + "' and DATE(radivac_drain_date) between '" + startDate + "' and '" + endDate + "' order by radivac_drain_date;";
