            ...
            for (int orc_i = 0; orc_i < orc.size(); orc_i++) {
                for (int dcg_i = 0; dcg_i < dcg1.size(); dcg_i++) {
                    ...
                    if (orcs.get(1).get(0).equals("T12115") && dcgs.get(2).get(2).equals("003")) {
                        ...
                        Date date = new Date();
                        String sql_ADW = "INSERT INTO wis_order_master (pmi_no, order_no, txn_type, hfc_cd, episode_date, encounter_date, order_date, order_by, discipline_cd, subdiscipline_cd, ordering_hfc_cd, ordering_discipline_cd, ordering_subdiscipline_cd, order_status, diagnosis_cd, created_by, create_date,txn_date) values ('" + t.getPmi_no() + "','" + allSeq.getSeq() + "','" + orcs.get(1).get(0) + "','" + orcs.get(12).get(0) + "','" + orcs.get(7).get(0) + "','" + orcs.get(7).get(0) + "','" + orcs.get(6).get(0) + "','" + orcs.get(9).get(0) + "','" + orcs.get(13).get(0) + "','" + orcs.get(14).get(0) + "','" + orcs.get(12).get(0) + "','" + orcs.get(15).get(0) + "','" + orcs.get(29).get(0) + "','0','-','" + orcs.get(9).get(0) + "','" + orcs.get(7).get(0) + "','" + dateFormat.format(date) + "')";
                        try {
                            ...
                            if (status_adw_master == true) {
                                ...
                                Date date2 = new Date();
                                String sql_adw_det = "INSERT INTO wis_order_detail (order_no, item_cd, episode_date, encounter_date, requestor_comments, admission_reason, order_by, order_date, ward_class_code, ward_id, bed_id, order_status, created_by, created_date,txn_date) values ('" + allSeq.getSeq() + "','-','" + dcgs.get(1).get(0) + "','" + dcgs.get(2).get(0) + "','" + dcgs.get(5).get(7) + "','-','" + dcgs.get(2).get(3) + "','" + dcgs.get(1).get(0) + "','-','" + dcgs.get(5).get(4) + "','-','0','" + dcgs.get(2).get(4) + "','" + dcgs.get(1).get(0) + "','" + dateFormat.format(date2) + "')";
