...
<tbody>
    ...
    <%
        ...
        if (process.equalsIgnoreCase("today")) {
            orderWhereClause = " AND date(lsm.order_date) = date(now()) ";
        } else if (process.equalsIgnoreCase("yesterday")) {
            orderWhereClause = " AND date(lsm.order_date) = DATE(NOW() - INTERVAL 1 DAY) ";
        } else if (process.equalsIgnoreCase("7")) {
            orderWhereClause = " AND (date(lsm.order_date) between SUBDATE(CURDATE(),7) and CURDATE() ) ";
        } else if (process.equalsIgnoreCase("30")) {
            orderWhereClause = " AND (date(lsm.order_date) between SUBDATE(CURDATE(),30) and CURDATE() ) ";
        } else if (process.equalsIgnoreCase("60")) {
            orderWhereClause = " AND (date(lsm.order_date) between SUBDATE(CURDATE(),60) and CURDATE() ) ";
        } else if (process.equalsIgnoreCase("custom")) {
            orderWhereClause = " AND (date(lsm.order_date) between STR_TO_DATE('" + fromDate + "','%d/%m/%Y') and STR_TO_DATE('" + toDate + "','%d/%m/%Y') ) ";
        }
