<%-- 
    Document   : WaitingTimeConsultLaboratoryTable
    Created on : Oct 26, 2018, 11:48:31 PM
    Author     : Shammugam
--%>
    ...
    <%
        ...
        if (filter.equals("month")) {
            ...
            sql = " SELECT DATE_FORMAT(que.episode_date, '%M %Y') AS BULAN, que.pmi_no, bio.PATIENT_NAME, que.user_id AS 'Doctor ID', IFNULL(doc.USER_NAME,'-') AS 'Doctor Name', "
                    //                  5                                       6                                   7                
                    + " que.episode_date AS 'Register Date', lis.encounter_date AS 'Consult Date', lis.order_date AS 'Laboratory Get Order', "
                    //                  8                                                                         
                    + " TIMESTAMPDIFF(MINUTE, lis.encounter_date, lis.order_date) AS 'Duration in minutes',   "
                    //                              9                                       10                                      11
                    + " DATE_FORMAT(que.episode_date,'%d/%m/%Y %H:%i:%s'), DATE_FORMAT(lis.encounter_date,'%d/%m/%Y %H:%i:%s'), DATE_FORMAT(lis.order_date,'%d/%m/%Y %H:%i:%s') "
                    // FROM PMS SQL
                    ...
                    + " JOIN lis_order_master lis ON (que.pmi_no = lis.pmi_no AND que.episode_date = lis.episode_date AND lis.hfc_to = que.hfc_cd) "
                    // JOIN USERS SQL
...
<script type="text/javascript" charset="utf-8">
    ...
    if ('<%=filter%>' === 'month') {
        ...
        prepend = '<div class="logo-hfc asset-print-img" style="z-index: 0; top: 0px; opacity: 1.0;">\n\
                    <img src="<%=mysqlhfc_cd.get(0).get(0)%>" style="text-align: center; height: 100%; " /></div> <div class="mesej"><br>Waiting Time Report (Consultation -> Laboratory)<br/><h5>For <strong>' + detailmonth + ' ' + detailyear + '</strong> </h5></div>\n\
        ...
        prepend = '<div class="logo-hfc asset-print-img" style="z-index: 0; top: 0px; opacity: 1.0;">\n\
                    <img src="<%=mysqlhfc_cd.get(0).get(0)%>" style="text-align: center; height: 100%; " /></div> <div class="mesej"><br>Waiting Time Report (Consultation -> Laboratory)<br/><h5>From <strong>' + startdate + '</strong>  To <strong>' + enddate + '</strong> </h5></div>\n\
                    <div class="info_kecik">\n\
                    <dd>Date: <strong><%=newdate%></strong></dd>\n\
                    <dd>Report ID: <strong>SAM-0004</strong></dd>\n\
                    </div> ';
            ...
            buttons: [
                {
                    ...
                    title: 'Waiting Time List (Consultation -> Laboratory)',
