<%-- 
    Document   : pis_setting_process
    Created on : Nov 2, 2018, 2:36:55 AM
    Author     : user
--%>
...
<%
    ...
    String query = "select status from adm_system_parameter where system_code = 'PISS' and parameter_code ='1' and hfc_cd='" + hfc_cd + "';";
    ...
    if (dataPara.size() < 1) { //No data yet, so we insert new row of data
        ...
        query = "Insert into adm_system_parameter(system_code, parameter_code, parameter_name, status, created_by, created_date, hfc_cd) "
                + "values('PISS', '1', 'PIS Dispense Type', '" + type + "', '" + creator + "', now(), '" + hfc_cd + "');";
        ...
        query = "Update adm_system_parameter set status='" + type + "' "
                + "where hfc_cd='" + hfc_cd + "' and system_code='PISS' and parameter_code='1'";
    ...
    if (success) {
        ...
        out.print("success");
        //set session
        session.setAttribute("PIS_PARAM", type);
