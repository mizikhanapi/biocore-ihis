...
<%
    ...
    Conn con = new Conn();
    //                          0            1            2               3                  4                5             6                     
    String query = "SELECT sd.order_no, sd.item_cd, sd.item_desc, sd.quantity, sd.released_quantity, sd.comment, sd.txn_date "
            // FROM RESULT TABLE
            + "FROM stk_order_detail sd "
            // JOIN ITEM TABLE
            + "LEFT JOIN stk_stock_item id ON (id.item_cd = sd.item_cd) AND id.hfc_cd = '" + hfc_cd + "' AND id.discipline_cd = '" + dis_cd + "'"
            // WHERE CONDITION
            + "WHERE sd.order_no = '" + order_no + "';";
...
%>
<tr style="text-align: left;" class="clickable_tr <%=evod%>">
